...
public class EnviSetup {
	 ...
	 * @param Optional	Cross Population #################
    ...
    private static HashMap<String, Object> setupArgs(String[] args, Log log)
    	...
    	parser.addArgument("-xp", "--cross_pop").choices("ACB", "ASW", "BEB", "CDX", "CEU","CHB", "CHS", "CLM",
                "ESN", "ESN", "FIN", "GBR", "GIH","GWD", "IBS", "ITU", "JPT", "KHV", "LWK", "MSL",
                "MXL", "PEL", "PJL", "PUR", "STU", "TSI", "TST", "YRI").help("Cross population.").help("Cross population. "
	    ...
	    //when out pop is defined but cross pop is undefined
	    if(parsedArgs.get("out_pop") != null && parsedArgs.get("cross_pop") == null) {
	    	String msg = "Error: Invalid population declaration, if outgroup population is defined "
	    			+ "cross pop needs to be defined as well";
	    	throw new IllegalInputException(log, msg);
	    }
	    
	    //default out_pop to cross population if not set
	    if(parsedArgs.get("out_pop") == null && parsedArgs.get("cross_pop") == null) {
	    	parsedArgs.put("cross_pop", "NO-XP");
	    	parsedArgs.put("out_pop", "NO-OP");
	    }
	    else if(parsedArgs.get("out_pop") == null) {
