...
public class StatsCalc {
	...
	private void setWindowStats() {
		...
		ws.setDAF(d.getDafStats(), d.getDafSNPs());
...
//				d_thrd.start();
				...
				synchronize(i_thrd, h_thrd, x_thrd, d_thrd, f_thrd);//add t_thrd, new_thrd
				...
				synchronize(i_thrd, h_thrd, d_thrd);
	...
	private void synchronize(StatsThread i_thrd, StatsThread h_thrd, StatsThread d_thrd) {

		for(;;) {
			if(i_thrd.isFinished() && h_thrd.isFinished() && d_thrd.isFinished())
				break;
			else
				continue;
		}
...
//			System.out.println(tp_win.getSnpListSize());
//			System.out.println(txin_win.getSnpListSize());
...
//				d = new dDAF();
	...
	private String getTargetXCrossWindowFileName() {
